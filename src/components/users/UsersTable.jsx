...
import { USER__DATA } from "../../constant";
...
export default function UsersTable() {
	...
	const [filteredUsers, setFilteredUsers] = useState(USER__DATA);

	const handleSearch = (e) => {
		const term = e.target.value.toLowerCase();
		setSearchTerm(term);
		const filtered = USER__DATA.filter(
			(user) =>
				...
				<table className="min-w-full divide-y divide-zinc-700">
					<thead>
						<tr>
							...
							<th className={tableHeadingClasses}>Email</th>
							<th className={tableHeadingClasses}>Role</th>
								...
								<td className="px-6 py-4 whitespace-nowrap">
									<div className="flex items-center">
										<div className="shrink-0 h-10 w-10">
											<div className="h-10 w-10 rounded-full bg-linear-to-r from-purple-400 to-blue-500 flex items-center justify-center text-white font-semibold">
												{user.name.charAt(0)}
											</div>
										</div>
										<div className="ml-4">
